...
* validation error slightly less than previous iteration (sub1)
...
GAM Summary:
* urb_v_rururban and now alprazolam_per_65_and_over not significant
...
Sub4 - remove at or below poverty - not informative/predictive
...
RMSE Error:
* Validation error went up slightly
...
RMSE Error:
* Validation error slightly worse and concurvity between mean income and poverty metric didn't change much, maybe wait and see to remove alprazolam?
...
GAM Summary:
* `alprazolam_per_65_and_over` now significant predictor
...
Model is now much, much worse - just go with model iteration 10 as the final formulation.
